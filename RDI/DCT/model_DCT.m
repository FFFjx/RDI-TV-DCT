function [ z1, z2, err_gather ] = model_DCT( K_diag, t_ML, r_ML, pra )
% The model of DCT regularization optimization.
...
for k=1:iteration
    if k==1
        z_pre = updateZ_DCT( inverse_M_diag , K_diag , operator );
    else
        z_pre = z;
    end 
    
    operator = updateU_DCT(K_diag, r_ML, t_ML, z_pre, operator, pra{1,1}, pra{1,2} );
    operator = updateD_DCT(K_diag, z_pre, operator);
    z = updateZ_DCT( inverse_M_diag , K_diag , operator );
    
    t = z(1:N, 1);
    r = z(N+1:2*N, 1);
    [ err, err1, err2 ] = loss_DCT( K_diag, t_ML, r_ML, t, r, ...
        pra{1,2}.tau_1, pra{1,2}.tau_2, pra{1, 1}.sigma );
    err_gather(1, k) = err;
    err_gather(2, k) = err1;
    err_gather(3, k) = err2;
end
